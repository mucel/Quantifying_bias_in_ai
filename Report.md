...
Based on research it was found that suface-level evaluations of the preformance of LLMs form both systems and user perspective is often done by using data sets, and with most they also created new metrics to evaluate the biases that the models exhibit, for example with the SterioSet created an idealized score that compared the model to an idealistic model (Sun et al., 2023, Kotek et al., 2023, Smith et al., 2022, Nadeem et al., 2020). Howevere, some maily focused on the evaluation of preformance by using these newly created scores (Sun et al., 2023) rather than focusing on the quantification of biases. Others, that did quantify biases, focused on a singular biase : gender, for the evaluation (Kotek et al., 2023). One method that was not rooted in data sets was a model-free approach, which is when probing is used as a prompting task in the hopes of analysing reponses withouth leveraging any specific knowlegde (Li et al., 2022). Sadly this is only useful to identify embedded linguistic properties, thus it is a surface-level analysis that misses propagated biases in the models and would be insufficient for causality analysis as it lacks the understanding of why and how biases occur. Another relitevely new approach is unanticipated bias detection trough the use of Uncertainty Qunatification and Explainable AI methods to detected less obvious implicit biases (Kruspe, 2024). Still in that study they mainly focus on how explainability can help the users identify bias. As the intent was to explore multiple chosen biases : gender and race, the focus was narrowed down to a popularly used approach for testing a models factual knowledge retrival called prompt probing (Jiang et al., 2020, Brown et al., 2020, Zhong et al., 2021). 
...
... 
Sadly this is only useful to identify embedded linguistic properties, thus it is a surface-level analysis that misses propagated biases in the models and would be insufficient for causality analysis as it lacks the understanding of why and how biases occur. 
